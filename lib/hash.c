...
** * the order order that those keys are inserted
...
**
**
** The attacker doesn't know the full state of the data structure, or any
** secret "salt" used to randomise it. If the attacker can figure some of this
** out, they can locally construct hash keys to send that will trigger
** pathological behaviour. They might need to brute force these locally, but
** this is an acceptable trade off to get to small network traffic with an
** outsized remote effect.
**
...
** hash code only needs to collide on those bits. Hence, it's important
** that the hash algorithm has a good final mixing stage
...
** timing of the array split, there's no need to conceal when the split happened
** Hence it's equivalently secure simply to ensure that for any level of "split"
** the iteration order gives no clue about the low (or high) $n bits of the
** full hash code used to pick the bucket.
**
** The hash code needs to stay the same as the hash splits (to avoid the
** performance hit of recomputing every key's hash code)
** Hence this code mixes a random 32 bit integer into the hash code before
** computing the bucks, and changes that integer each time the array splits.
**
